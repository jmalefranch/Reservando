...
Listado.prototype.buscarRestaurante = function(id) {
    var buscadoID = this.restaurantes.find(elemento => elemento.id === id);
    if(buscadoID != null){
        return buscadoID;
    }
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    var ciudades = this.restaurantes.map(elemento => {return elemento.ubicacion});
    var ciudadesSinRepetir = eliminarRepetidos(ciudades);
...
Listado.prototype.obtenerRubros = function() {
    var rubros = this.restaurantes.map(elemento => {return elemento.rubro});
...
Listado.prototype.obtenerHorarios = function() {
    ...
    var arregloH = this.restaurantes.map(elemento => {return elemento.horarios});
    var horariosRepetidos = [];
    arregloH.forEach(function(element) {
        element.forEach(function(hor) {
            horariosRepetidos.push(hor)
        });
    });

    var horariosSinRepetir = eliminarRepetidos(horariosRepetidos);

    return horariosSinRepetir.sort();
}

//Funcione que elimina elementos repetidos de un array
var eliminarRepetidos = function(array){
